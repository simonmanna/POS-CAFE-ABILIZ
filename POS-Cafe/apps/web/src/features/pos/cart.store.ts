...
export const useCartStore = create<CartState>()(
  persist(
    (set) => ({
      ...
      addLine: (line) => {
        set((state) => {
          // P10: key on (product|sku) + taxInclusive so the cashier can
          // add the same product twice with different tax handling.
          const key = (line.productId ?? line.sku?.toLowerCase() ?? '') +
            (line.taxInclusive ? '|incl' : '');
          const existing = state.lines.find(
            (l) => ((l.productId ?? l.sku?.toLowerCase() ?? '') + (l.taxInclusive ? '|incl' : '')) === key,
          );
          if (existing) {
            return {
