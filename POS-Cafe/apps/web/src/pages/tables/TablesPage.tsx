 ...
 * for archive confirmation. Polls /pos/tables every 10s; stats every 5s.
 */
...
export const TablesPage: React.FC = () => {
  ...
  async function doClean(id: string, number: number) {
    try {
      await clean.mutateAsync(id);
      toast.success(`T${number} marked clean`);
    } catch (e: any) {
      toast.error(e?.response?.data?.message ?? 'Failed to clean table');
    }
  }
  ...
  return (
    <div className="page-wrap space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
        ...
        <StatTile label="Needs Cleaning" value={stats?.dirty ?? 0}        tone="from-amber-500 to-yellow-500" />
        <StatTile
          label="Occupancy"
      ...
      <div className="glass-card p-3 flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {(['all', 'available', 'occupied', 'reserved', 'dirty', 'out_of_service'] as const).map(
            (k) => (
              <button
                key={k}
                onClick={() => setFilter(k)}
                ...
                <CardContent className="relative p-4">
                  ...
                  >
                    ...
                    {table.status === 'dirty' ? (
                      <Button
                        size="sm"
                        className="h-7 text-[11px] bg-emerald-600 hover:bg-emerald-700 btn-shine"
                        onClick={() => doClean(table.id, table.number)}
                      >
                        <Brush className="w-3 h-3 mr-1" /> Mark clean
                      </Button>
                    ) : null}
                    <Button
                      variant="outline"
