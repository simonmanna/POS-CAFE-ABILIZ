...
import { SnapshotRebuildService } from '../../modules/accounting/reporting/snapshots/snapshot-rebuild.service';
import { TieOutService } from '../../modules/accounting/reporting/tieout.service';
 ...
 *
 * Runs in-process for the single-replica beta. On boot it schedules itself to
 * run daily at the configured hour (default 02:00 UTC). Operators can trigger
 * an immediate rebuild via the admin endpoint at any time.
 *
 * For multi-replica deployments replace this with a distributed scheduler
 * (BullMQ or pg-boss); out of scope for the beta.
 */
@Injectable()
export class SnapshotCronWorker implements OnApplicationBootstrap, OnModuleDestroy {
  ...
  /** Trigger an immediate rebuild — wired to a future admin endpoint. */
  ...
  private scheduleNext(): void {
    ...
    const now = new Date();
    const next = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), this.runHourUtc, 0, 0));
