...
export class PosReservationsService {
  ...
  async cancel(id: string) {
    ...
    return this.prisma.client.$transaction(async (tx: any) => {
      ...
      // Only flip the table back if no other pending reservation covers it.
      const stillReserved = await tx.posTableReservation.findFirst({
        where: {
  ...
  async complete(id: string) {
    ...
    return this.prisma.client.$transaction(async (tx: any) => {
      ...
      return tx.posTableReservation.update({
        where: { id },
   ...
   * always reflects upcoming bookings.
   */
