...
class StatusBody {
  @ApiProperty({ enum: ['available','occupied','reserved','dirty','out_of_service'] })
  @IsIn(['available','occupied','reserved','dirty','out_of_service'])
...
export class PosTablesController {
  ...
  @Post(':id/clean')
  @RequirePermissions('tables:clean')
  clean(@Param('id') id: string) {
    return this.svc.clean(id);
  }
  ...
  @Post(':id/mark-dirty')
  @RequirePermissions('tables:clean')
  markDirty(@Param('id') id: string, @Body() dto: { documentId?: string }) {
    return this.svc.markForCleaning({ tableId: id, documentId: dto.documentId });
  }
}