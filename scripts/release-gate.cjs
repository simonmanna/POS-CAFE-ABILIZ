...
if (!DATABASE_URL) {
  ...
  jest('full jest suite (DB required)', null, DATABASE_URL, [
    /pos-money-foundations|pos-sale-pipeline|pos-store-credit-issuance|pos-cash-flow-go-live|pos-cash-flow-day/,
...
if (!POS_TEST_DATABASE_URL || !/^\/pos_stage1_\d+$/.test(new URL(POS_TEST_DATABASE_URL).pathname)) {
  ...
  jest('isolated money suites', 'pos-money-foundations|pos-sale-pipeline|pos-store-credit-issuance|pos-cash-flow-go-live|pos-cash-flow-day', POS_TEST_DATABASE_URL, []);
}
