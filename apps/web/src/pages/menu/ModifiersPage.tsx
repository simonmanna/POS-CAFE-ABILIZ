...
export default function ModifiersPage() {
  ...
  // Inline modifier edit state
  const [editModifier, setEditModifier] = useState<{ id: string; name: string; priceDelta: number; isDefault: boolean } | null>(null);
  const [editModName, setEditModName] = useState('');
  const [editModPrice, setEditModPrice] = useState('');
  const [editModDefault, setEditModDefault] = useState(false);

  // beforeunload when inline editing
  useEffect(() => {
  ...
  const { data: menuItemsRsp } = useMenuItems({ page: 1, pageSize: 100, search: menuItemSearch });
    ...
    <div className="p-2 max-w-[1800px] mx-auto space-y-2">
      <div className="mb-2">
        ...
        <Button onClick={() => setShowNewGroup(true)} style={{ background: '#4f46e5' }}>
          <Plus className="h-4 w-4 mr-1" /> New Modifier/Add-on Group
        </Button>
      </div>
          ...
          {groups.map((g) => (
            <div key={g.id} className="rounded-lg border border-slate-200 bg-white p-2.5">
              ...
              <div className="flex flex-wrap gap-1.5 mb-1.5">
                ...
                {g.modifiers.map((m) =>
                  editModifier?.id === m.id ? (
                    <div key={m.id} className="flex items-center gap-2 w-full border border-indigo-300 rounded-lg bg-indigo-50/80 p-3">
                      <Input value={editModName} onChange={(e) => setEditModName(e.target.value)} className="h-9 text-sm flex-1 min-w-0" placeholder="Option name" />
                      <div className="w-28">
                        <Input type="number" value={editModPrice} onChange={(e) => setEditModPrice(e.target.value)} className="h-9 text-sm" placeholder="Price" />
                      </div>
                      <label className="flex items-center gap-1.5 text-sm font-medium text-slate-700 whitespace-nowrap cursor-pointer select-none">
                        <input type="checkbox" checked={editModDefault} onChange={(e) => setEditModDefault(e.target.checked)} className="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" /> Default
                      </label>
                      <Button onClick={async () => {
                        await updateModifier.mutateAsync({ id: m.id, name: editModName.trim(), priceDelta: Number(editModPrice) || 0, isDefault: editModDefault });
                        setEditModifier(null);
                        toast.success('Modifier updated');
                      }} disabled={updateModifier.isPending} style={{ background: '#16a34a' }} className="h-9 text-sm px-3">
                        <Check className="h-4 w-4 mr-1" /> Save
                      </Button>
                      <Button variant="ghost" onClick={() => setEditModifier(null)} className="h-9 text-sm px-3" disabled={updateModifier.isPending}>Cancel</Button>
                    </div>
                  ) : (
                    <span key={m.id} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg border border-slate-200 bg-slate-50 text-sm font-semibold text-slate-700 shadow-sm">
                      {m.isDefault ? <Star className="h-4 w-4 text-amber-500" /> : null}
                      {m.name}
                      <span className="font-mono text-slate-500">{m.priceDelta === 0 ? '' : (m.priceDelta > 0 ? '+' : '') + fmt(m.priceDelta)}</span>
                      <button onClick={() => { setEditModifier(m); setEditModName(m.name); setEditModPrice(String(m.priceDelta)); setEditModDefault(m.isDefault); }} className="p-1 text-slate-400 hover:text-indigo-600 rounded transition-colors" title="Edit option">
                        <Pencil className="h-3.5 w-3.5" />
                      </button>
                      <button onClick={() => removeOption(m.id)} className="p-1 text-slate-400 hover:text-rose-600 rounded transition-colors font-bold leading-none" title="Remove option">
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                    </span>
                  )
              ...
              {optFor === g.id ? (
                 <div className="flex flex-wrap items-end gap-2 border-t border-slate-100 pt-2">
                  ...
                  <Button onClick={() => submitOption(g.id)} disabled={createModifier.isPending} style={{ background: '#16a34a' }} className="h-9 text-sm px-4">
                    <Check className="h-4 w-4 mr-1" /> Add
                  </Button>
                  <Button variant="ghost" onClick={() => { setOptFor(null); setOptName(''); setOptPrice(''); setOptDefault(false); }} className="h-9 text-sm">Cancel</Button>
                </div>
              ) : (
                <button type="button" onClick={() => setOptFor(g.id)} className="text-sm font-semibold text-indigo-600 hover:text-indigo-800 inline-flex items-center gap-1.5 py-1">
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-2.5">
            ...
            <div className="space-y-2">
              <div>
                ...
                <Input placeholder="Search menu items…" value={menuItemSearch}
      ...
      <Dialog open={!!renameTarget} onOpenChange={(o) => { if (!o) setRenameTarget(null); }}>
        <DialogContent className="sm:max-w-[380px]">
          <DialogHeader>
            ...
            <DialogDescription>Enter a new name for this modifier group.</DialogDescription>
          </DialogHeader>
              ...
              <Input value={renameValue} onChange={(e) => setRenameValue(e.target.value)}
                placeholder="Group name" autoFocus onKeyDown={(e) => {
                  if (e.key === 'Enter' && renameValue.trim() && renameTarget) {
                    updateGroup.mutateAsync({ id: renameTarget.id, name: renameValue.trim(), expectedVersion: groupVersions[renameTarget.id] })
                      .then(() => { toast.success('Group renamed'); setRenameTarget(null); })
          ...
          <DialogFooter>
            ...
            <Button onClick={() => {
              ...
              updateGroup.mutateAsync({ id: renameTarget.id, name: renameValue.trim(), expectedVersion: groupVersions[renameTarget.id] })
                .then(() => { toast.success('Group renamed'); setRenameTarget(null); })
                .catch(handleConflict);
            }} disabled={!renameValue.trim() || updateGroup.isPending} style={{ background: '#4f46e5' }}>Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
