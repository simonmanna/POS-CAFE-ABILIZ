...
(enabled ? describe : describe.skip)('Lakeview Café & Grill — M1/M2 business simulation (D0–D1)', () => {
  ...
  run('D1-L-001', 'Table 5: KOT1 full order → add espresso round → KOT2 delta only → pay card', 'P0', async (rec) => {
    ...
    rec.rules.push({ rule: 'KOT-04 table released after settlement', status: (t.status === 'available') ? 'PASS' : 'FAIL', actual: t.status });
  ...
  run('D1-L-002', 'Table 8 split by item: guest 1 cash, guest 2 MTN; double settle is a no-op', 'P0', async (rec) => {
    ...
    rec.rules.push({ rule: 'KOT-04 table not released while a split bill is unpaid', status: !(mid.status === 'available') ? 'PASS' : 'FAIL', actual: mid.status });
    ...
    rec.rules.push({ rule: 'KOT-04 table released after all splits paid', status: (end.status === 'available') ? 'PASS' : 'FAIL', actual: end.status });
