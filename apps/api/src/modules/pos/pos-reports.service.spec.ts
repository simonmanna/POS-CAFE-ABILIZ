...
describe('PosReportsService (financial accuracy)', () => {
  ...
  describe('xReport', () => {
    ...
    it('reports net revenue ex-tax, gross, tax and refunds separately', async () => {
      const r = await svc.xReport('s1');
      ...
      expect(r.totals.refundsTotal).toBe('10.00');
      expect(r.totals.netSales).toBe('167.00');    // 177 − 10
    });
