...
export class PaymentService {
  ...
  async void(id: string) {
    return this.prisma.client.$transaction(async (tx: any) => {
      const payment = await tx.payment.findFirst({ where: { id }, include: { allocations: true } });
      ...
      if (payment.journalEntryId) {
        await this.posting.reverse(
          ...
          { description: `Void of ${payment.paymentNumber}` },
          tx,
        );
      }
      ...
      for (const alloc of payment.allocations) {
        const doc = await tx.document.findFirst({ where: { id: alloc.documentId } });
        if (doc) {
          const newPaid = (doc.amountPaid as Prisma.Decimal).minus(alloc.amount);
          const newResidual = (doc.amountResidual as Prisma.Decimal).plus(alloc.amount);
          const paymentStatus = this.residualStatus(doc.totalAmount, newResidual);
      ...
      await tx.payment.updateMany({
        where: { id: payment.id },
        data: { status: 'cancelled', allocatedAmount: 0, unallocatedAmount: 0 },
      });
      ...
      return tx.payment.findFirst({ where: { id: payment.id } });
    });
  }
